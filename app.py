...
# 1. Try to get real-time GPS automatically
loc = get_geolocation()

# Default to Iloilo if GPS is not yet available
...
if st.button("🚀 Start 1km Radius Analysis"):
    ...
    folium.Marker([lat, lon], popup="Your Position").add_to(m)
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.info(f"**Analysis Summary:** Location {lat:.4f}, {lon:.4f} is in a **{risk_level}** risk zone. The temperature is currently **{avg_lst:.1f}°C**.")
    with c2:
    ...
    st.write("---")
    st.write("Click the button above to begin the satellite analysis for your current location.")
